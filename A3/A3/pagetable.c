...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// victim page that has to be swapped out
		pgtbl_entry_t* victim_page = coremap[frame].pte;

		// if victim page has been modified but not saved
		if(victim_page->frame & PG_DIRTY){
			// Swap page out before swapping new page in
			int swap_offset = swap_pageout(frame, victim_page->swap_off);

			// Check if the swap out is successful
			if(swap_offset == INVALID_SWAP){
				printf("Failed to swap page out!\n");
				exit(1);
			}

			victim_page->swap_off = swap_offset;

			// Set the ONSWAP bit to indicate the page has been evicted to swap
		...
		// Set the VALID bit to indicate the page is now not on memory
		victim_page->frame &= ~PG_VALID;
		
		// Set the DIRTY bit to indicate the page is fresh
...
char *find_physpage(addr_t vaddr, char type) {
	...
	// initialize second level page table if not initialized
	if (!(pgdir[idx].pde & PG_VALID)) {
		pgdir[idx] = init_second_level();
	}

	// Get pointer to the 2nd-level page table
	pgtbl_entry_t *pgtbl = (pgtbl_entry_t *) (pgdir[idx].pde);

	// Use vaddr to get index into 2nd-level page table and initialize 'p'
	// Get corresponding page table entry for vaddr
	...
	if (!(p->frame & PG_VALID)){
		...
		// Page already on swap
		}else{
			// Swap page into the memory
			if(swap_pagein(frame, p->swap_off)){
				printf("Failed to swap page in!\n");
				exit(1);
			}
			p->frame= frame << PAGE_SHIFT;
		}
