...
{
	// delete the pid from lists in all table entries
	// using lock to guarantee synchronization
	spin_lock(&my_table_lock);
	del_pid(current->pid);
	spin_unlock(&my_table_lock);
	// call original exit funtion to exit
...
asmlinkage long interceptor(struct pt_regs reg) {
	// reg.ax for storing syscall number 
	// all pids should be monitored for the syscall if monitored == 2
	if(table[reg.ax].monitored == 2){
		// log message for those pids not in the blacklist
		// *************** this is for bonus ***************
		if(!check_pid_monitored(reg.ax, current->pid))
			log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}else if(table[reg.ax].monitored == 1){
		// just simply check if it's monitored to decide if log is needed
		if(check_pid_monitored(reg.ax, current->pid))
			log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
	}
	// call original function that was being intercepted
	return (*table[reg.ax].f)(reg);
}

asmlinkage long start_intercept(int syscall) {
	// only root can intercept the syscall
	if(current_uid() != 0) return -EPERM;
	// if the syscall has already been intercepted
	if(table[syscall].intercepted == 1 ) return -EBUSY;
	// save original syscall and replace it with the interceptor
	spin_lock(&my_table_lock);
...
asmlinkage long start_release(int syscall) {
	// only root can intercept the syscall
	if(current_uid() != 0) return -EPERM;
	// if the syscall has not been intercepted yet
...
asmlinkage long start_monitor(int syscall, int pid) {
	// check if the pid is valid
	if(pid < 0 || (pid != 0 && !pid_task(find_vpid(pid), PIDTYPE_PID))) return -EINVAL;
	// cmd is not issued by the root or the owener of the process
	if (current_uid() != 0) {
      	if (pid == 0) return -EPERM;
      	else if (check_pids_same_owner(current->pid, pid) != 0) return -EPERM;
	}
	// the syscall has not been interrputed
	if(table[syscall].intercepted == 0) return -EINVAL;
	// the syscall has already been monitored for the pid and not all pids are monitored at first
	if(check_pid_monitored(syscall, pid) && table[syscall].monitored == 1) return -EBUSY;
	spin_lock(&my_table_lock);
	if(pid == 0){
		// reinitialize my_list in table[syscall] to be a blacklist(empty initially), so all pids can pass and get logged
		destroy_list(syscall);
		// monitor all pids for the syscall and use table[syscall].my_list as a blacklist, initially empty
		table[syscall].monitored = 2;
	}else{
		// if all has been monitored, remove the pid out of the blacklist
		if(table[syscall].monitored == 2){
			if(!del_pid_sysc(pid, syscall)){
				spin_unlock(&my_table_lock);
				return 0;
			}
			// the syscall has already been monitored for the pid and all pids are monitored at first
			return -EBUSY;
		}
		// monitor not all pids for the syscall, then use table[syscall].my_list to record the pid being monitored
...
asmlinkage long stop_monitor(int syscall, int pid) {
	// check if the pid is valid
	if(pid < 0 || (pid != 0 && !pid_task(find_vpid(pid), PIDTYPE_PID))) return -EINVAL;
	// cmd is not issued by the root or the owener of the process
	if (current_uid() != 0) {
	...
	// the syscall has not been intercepted or monitored yet
	if(table[syscall].intercepted == 0) return -EINVAL;
	spin_lock(&my_table_lock);
	if(pid == 0){
		destroy_list(syscall);
	}else{
		// can not stop a syscall that is not being monitored
		if(check_pid_monitored(syscall, pid) == 0){
			spin_unlock(&my_table_lock);
			return -EINVAL;
		}
		// remove the pid from table[syscall].my_list if not all pids are monitored
		...
		// a specific pid to be removed from monitored list (implemented in a reverse way)
		// i.e. we keep a blacklist in table[syscall].my_list that record pids not being monitored
		if(table[syscall].monitored == 2){
			if(add_pid_sysc(pid, syscall)){
				spin_unlock(&my_table_lock);
				return -ENOMEM;
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(cmd == REQUEST_SYSCALL_INTERCEPT) return start_intercept(syscall);
	
	// release the syscall 
	if(cmd == REQUEST_SYSCALL_RELEASE) return start_release(syscall);

	// monitor the syscall for the pid
	if(cmd == REQUEST_START_MONITORING) return start_monitor(syscall, pid);

	// stop monitoring the syscall for the pid
	if(cmd == REQUEST_STOP_MONITORING) return stop_monitor(syscall, pid);

	// if cmd is nod valid , return -EINVAL
	return -EINVAL;
}
...
static int init_function(void) {
	...
	for(sysc=1; sysc<NR_syscalls; sysc++){
		table[sysc].f = NULL;
	...
	spin_unlock(&my_table_lock);
	// put my_syscall and my_exit_group into sys_call_table and store corresponding original calls
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
...
{        
	int sysc;
	spin_lock(&my_table_lock);
	// destroy my_list in all table entries (all syscalls) when exiting the funcion
	for(sysc=1; sysc<NR_syscalls; sysc++){
		destroy_list(sysc);
	}
	spin_unlock(&my_table_lock);
	// restore the original syscall back to the sys_call_table
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
